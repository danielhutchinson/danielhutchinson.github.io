...
is used to call `execute` on whatever command the class has assigned. With this change, the `RemoteControlButton` no longer needs to know about the Television. We have successfully decoupled these constructs from one another.
